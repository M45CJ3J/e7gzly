...
class Ticket extends Service_Employee implements Operations
{
    ...
    {
        $result = $this->RUNSearch("SELECT sum( CASE WHEN ( br_service_id = ? AND receipt_no < (SELECT receipt_no FROM tickets WHERE user_id = ? AND br_service_id = ? AND status = 'waiting')) THEN 1 ELSE 0 END ) AS waiting FROM tickets", [$this->br_service_id, $this->user_id, $this->br_service_id]);
