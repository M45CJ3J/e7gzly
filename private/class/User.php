...
class User extends Database implements Operations
{
    ...
    {
        $result = $this->RUNSearch("SELECT * FROM users  WHERE org_id = ? AND branch_id = ? and user_type_id = 2", [$this->org_id, $this->branch_id]);
        return $result->rowCount() > 0 ? "This Branch already has a manager!" : false;
    }
    ...
    {
        $result = $this->RUNSearch("SELECT SUM(branch_emp) as employees, SUM(org_managers) as managers FROM count_employees WHERE org_id = ? GROUP BY org_id
        ", [$this->org_id]);
    ...
    {
        $result = $this->RUNSearch("SELECT * FROM users WHERE email =? OR phone = ? ", [$this->email, $this->phone]);
