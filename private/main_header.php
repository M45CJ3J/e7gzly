...
<body>
  <section id="particles-js" class="h-md-75 py-5">
    <script>
      // alert(window.location.pathname.includes('php'));
      if (!(window.location.pathname.includes('php') && window.location.pathname.includes('index.php'))) {
        document.querySelector('#particles-js').removeAttribute('id');
      }
    </script>

    <header class="w-100 pt-4 ">
      <a href="index.php" class="site-logo text-primary text-uppercase font-weight-bold pb-3" aria-label="homepage">E7GZLY</a>
      <nav class="main-nav ">
        <ul class="nav-list ">
          <?php
          if (strpos($_SERVER['PHP_SELF'], 'php') && strpos($_SERVER['PHP_SELF'], 'index.php')) :
            ?>
          ...
          <?php if (isset($_SESSION['user_id']) && $_SESSION['user_type'] == 1) : ?>
            ...
            <li class="nav-item dropdown">
              ...
              <div id="name-menu" class="dropdown-menu bg-transparent text-white" aria-labelledby="navbarDropdown">
    ...
    <?php if (strpos($_SERVER['PHP_SELF'], 'php') && strpos($_SERVER['PHP_SELF'], 'index.php')) : ?>
      <div id="home" class="w-100 h-100 d-flex justify-content-center text-center">
        <div class="home-text  mb-3 px-2 ">
          <h2 class="hero-text mb-5 font-weight-bold">Save Time, Save Money.</h2>
          <a class="button home-book button-accent " href="#" data-toggle="modal" data-target="#modal-login">Book</a>
        </div>
      </div>
    <?php endif; ?>
  </section>